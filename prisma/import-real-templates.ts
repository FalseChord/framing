...
async function main() {
  ...
  await prisma.template.upsert({
    ...
    update: {},
    create: {
      id: "real-template-matching",
      category: "媒合信",
      subject:
        "[加惠心理諮商]" +
        "[只有 青壯]諮商媒合信 (青壯方案)[/只有]" +
        "[只有 重大災害]諮商媒合信 (重大災害心理支持方案)[/只有]" +
        "[只有 EAP]EAP諮商媒合信[/只有]" +
        "[只有 公益]公益諮商媒合信[/只有]" +
        "[除外 青壯、重大災害、EAP、公益]諮商媒合信[/除外]",
      body:
        "{{caseRef}}，您好：\n\n" +
        "[只有 一般]" +
        ...
        "再麻煩請您回信或來電確認晤談時間。" +
        "[/只有]",
      variants: JSON.stringify(["一般", "伴侶", "青壯", "重大災害", "EAP", "公益"]),
      requiredFields: JSON.stringify(["caseRef", "therapistName", "sessionSlots", "fee", "eapPlanName"]),
      updatedById: admin.id,
    },
  });

  await prisma.template.upsert({
    where: { id: "real-template-preparation" },
    update: {},
    create: {
      id: "real-template-preparation",
      category: "準備信",
      subject:
        "[只有 青壯][加惠心理諮商]諮商準備信 (青壯方案)[/只有]" +
        "[只有 重大災害][加惠心理諮商]諮商準備信(重大災害心理支持方案)[/只有]" +
        "[只有 EAP][加惠基金會]EAP諮商準備信[/只有]" +
        "[只有 公益][加惠心理諮商]公益諮商準備信[/只有]" +
        "[除外 青壯、重大災害、EAP、公益][加惠心理諮商]諮商準備信[/除外]",
      body:
        "{{caseRef}}，您好：\n\n" +
        "[只有 一般]" +
  ...
  console.log("Real template content imported (媒合信, 準備信, 線上諮詢預約確認信, 通訊諮商預約確認信).");
}
